        ...
        <a target="_blank" href="item.php?id={$row['product_id']}" ><img width ='100' src="../kresources/{$product_photo}" alt=""></a>
            <div class="caption">
               ...
               <h4> <a target="_blank" href="item.php?id={$row['product_id']}" >{$row['product_title']}</a></h4>
                 ...
                 </div> <a class="btn btn-primary" target="_blank" href="..\kresources\cart.php?add={$row['product_id']}" text-align:center>Thêm vào giỏ hàng</a> </div>
 </div>
DELIMETER;
        echo $products;
    }
 ...
 <div class="col-md-3 col-sm-6 hero-feature">
                <div class="thumbnail">
                    ...
                    <div class="caption">
                        ...
                        <p>{$row['short_desc']}</p>
                        <p>
                            <a href="..\kresources\cart.php?add={$row['product_id']}" class="btn btn-primary">Mua ngay!</a> <a href="item.php?id={$row['product_id']}" class="btn btn-default">More Info</a>
                        </p>
                    </div>
                </div>
            </div>
...
function login_user()
{
    if (isset($_POST['submit'])) {
        ...
        if (mysqli_num_rows($query) == 0) {
            ...
            if ($user_level == 2) {
                redirect("index.php"); // Redirect to index.php if user_level is 2 (admin)
            } else if ($user_level == 1) {
                $_SESSION['username'] = $username;
                $_SESSION['user_id'] = $row['user_id'];
                redirect("index_user.php"); // Redirect to index1.php if user_level is 1 (user)
            }
        }
    }
}
    ...
    while ($row = fetch_array($category_query)) {
        ...
        if ($username === $_SESSION['username']) {
            $user = <<<DELIMETER
                ...
                <td>{$last_name}</td>
                 <td>{$email}</td>
                    <td><img src="{$image_path}" alt="User Image"></td>
                    </tr>
            DELIMETER;
...
function display_address()
{
    ...
    while ($row = fetch_array($category_query)) {
        ...
        <tr>
            ...
            <td>{$ward}</td>
            <td>{$address}</td>
            <td><a class="btn btn-danger" href="..\..\kresources\ktemplates\backend\delete_address.php?id={$row['id']}">
            <span class="glyphicon glyphicon-remove"></span></a></td>
            <td><a class="btn btn-danger" href="..\..\kresources\ktemplates\backend\popup_address.php?id={$row['id']}">
    <span class="glyphicon glyphicon-remove"></span></a></td>
...
<tr><td>{$address},{$ward},{$district},{$province}
</td></tr>
...
function edit1()
{
    if (isset($_POST['update_user'])) {
        ...
        $user_level = 1; // Đặt mặc định là 1
        ...
        echo "<script>alert('Dữ liệu đã được cập nhật thành công!'); window.location='index_user.php?users';</script>";
...
//$final_destination = UPLOAD_DIRECTORY . DS . $user_photo;
        ...
        $query = query("INSERT INTO users(user_level,first_name,last_name,username,email,password) VALUES('{$user_level}','{$first_name}','{$last_name}','{$username}','{$email}','{$password}')");
        confirm($query);
...
function register_user()
{
    if (isset($_POST['register_user'])) {
        ...
        if (mysqli_num_rows($query) > 0) {
            ...
            $query = query("INSERT INTO users(user_level,first_name,last_name,username,email,password) 
            VALUES('{$user_level}','{$first_name}','{$last_name}','{$username}','{$email}','{$password}')");
            confirm($query);
            set_message("USER CREATED");
            redirect("login.php");
        }
...
function address()
{
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Tiến hành xử lý lưu thông tin địa chỉ vào cơ sở dữ liệu hoặc làm những việc khác
        // ...
...
function add_address()
{
    if (isset($_POST['add_address'])) {
        ...
        if (empty($fullname) || empty($phone) || empty($province) || empty($district) || empty($ward) || empty($address)) {
            echo "<p class='bg-danger'>Các trường không được để trống</p>";
        } else {
